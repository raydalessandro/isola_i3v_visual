...
| `audit_5_timeline.py` | Ordine temporale-causale (sola lettura): semi (origine → eventi effettivi → fioritura), callback (registrati dopo l'origine), incroci storia↔seme/callback, sid del quote_tracker. Severo sull'**effettivo** (errore), morbido sull'**intento** (warning: bloom fuori target, echi post-fioritura). Baseline `timeline_*` in known_issues. | ✅ |
